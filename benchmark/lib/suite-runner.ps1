...
function Write-SuiteAttempt {
    ...
    Write-BenchmarkAttemptRecord -Root $root -Path $AttemptLedger -MeasurementPurpose $Mode `
...
function Invoke-NativeBenchmark {
    ...
    $env:NPU_INFERENCE_BENCH_PUBLISH_ROOT = $root
    ...
    if ($Mode -eq "accuracy-quick" -and $Workload.kind -eq "asr") {
        ...
        $record = New-BenchmarkAccuracyRecord -Root $root -Workload $Workload `
