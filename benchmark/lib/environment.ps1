...
function Get-BenchmarkEnvironmentSnapshot {
    ...
    try {
        ...
        if ($battery) {
            $batteryDetails = [ordered]@{
                device_id = Get-BenchmarkAnonymousId `
                    -Namespace "battery-device" -Value "$($battery.DeviceID)"
    ...
    try {
        foreach ($zone in @(Get-CimInstance -Namespace root/wmi `
            ...
            $thermalZones += [ordered]@{
                instance_id = Get-BenchmarkAnonymousId `
                    -Namespace "thermal-zone" -Value "$($zone.InstanceName)"
    ...
    $snapshot = [ordered]@{
        ...
        runner_id = $RunnerId
        executable = [ordered]@{
            path = ConvertTo-BenchmarkPublishedPath -Root $Root -Path $Exe
            sha256 = (Get-FileHash -LiteralPath $Exe -Algorithm SHA256).Hash.ToLowerInvariant()
        }
        host = [ordered]@{
            host_id = Get-BenchmarkHostId
        ...
        power = [ordered]@{
            source = $powerSource
            active_plan = Get-BenchmarkPublishedPowerPlan $powerPlan
...
function Write-BenchmarkCompilationProvenance {
    ...
    $record = [ordered]@{
        ...
        artifact_path = ConvertTo-BenchmarkPublishedPath -Root $Root -Path $artifact
    ...
    $record = ConvertTo-BenchmarkPublishedValue -Root $Root -Value $record
