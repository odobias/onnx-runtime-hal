...
| `npu_inference_bench/runtime/execution_plan.hpp` | Workload-neutral execution-plan contract |
...
`ort_ep.hpp` and `ort_offload.hpp` ship only as implementation details required
by the packaged translation units. They are not public API.

## Package an install tree

The cross-repository contract is `GenDigital.OnnxRuntimeHal.Source`: workload-neutral
headers, two implementation translation units, and native MSBuild integration
(no model weights, adapters, benchmark code, or runtime binaries). See
[`packaging/onnx-runtime-hal-source/README.md`](../packaging/onnx-runtime-hal-source/README.md).
The package also carries the execution-plan schema and FakeAudio whole/split examples.
