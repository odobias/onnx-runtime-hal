...
// scripts/experiments/onnx_decode.py (same prompt, suppress, greedy logic).
...
public:
    explicit OrtDynamicEngine(const EngineOptions& options)
        : env_(ORT_LOGGING_LEVEL_WARNING, "whisper_hal_ort_dynamic"), options_(options) {
        ...
        // demote to GPU/CPU -- the honest, documented HAL behavior.
        ...
        for (size_t i = 0; i < chain.size(); ++i) {
            ...
            try {
                ...
                decoder_past_.reset();
                if (i + 1 < chain.size()) {
                    std::cerr << "[whisper-hal] dynamic EP '" << provider << "' unavailable ("
...
std::unique_ptr<IWhisperEngine> create(const EngineOptions&) {
    throw std::runtime_error(
        "ONNX Runtime dynamic backend not compiled. Build with WHISPER_HAL_ORT and link ONNX Runtime.");
}
