...
Device RuntimeContext::probe_best_device() {
    ...
    for (Ort::ConstEpDevice candidate : env.GetEpDevices()) {
        const auto device_type = candidate.Device().Type();
        if (device_type == OrtHardwareDeviceType_NPU) return Device::NPU;
        if (device_type == OrtHardwareDeviceType_GPU) saw_gpu = true;
    }
    if (saw_gpu) return Device::GPU;
#else
    for (const std::string& provider : Ort::GetAvailableProviders()) {
        const std::string name = ort_common::lower(provider);
        if (name.contains("dml") || name.contains("directml")) return Device::GPU;
    }
#endif
