...
std::string anonymous_external_path(const std::string& path) {
    ...
    result << "external:" << std::hex << std::setfill('0') << std::setw(16) << hash;
    return result.str();
}

std::string published_path(const std::string& path) {
    namespace fs = std::filesystem;
    const std::string root = environment_value("NPU_INFERENCE_BENCH_PUBLISH_ROOT");
    if (path.empty() || root.empty()) return path;

    std::error_code ec;
    const fs::path root_path = fs::absolute(fs::path(root), ec).lexically_normal();
    if (ec) return anonymous_external_path(path);
    const fs::path full_path = fs::absolute(fs::path(path), ec).lexically_normal();
    if (ec) return anonymous_external_path(path);

    const fs::path relative = full_path.lexically_relative(root_path);
    if (!relative.empty()) {
        const auto first = relative.begin();
        if (first == relative.end() || *first != "..") {
            return "${REPO_ROOT}/" + relative.generic_string();
        }
    }
    return anonymous_external_path(full_path.generic_string());
}
