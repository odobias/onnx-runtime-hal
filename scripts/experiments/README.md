...
## C++ classifier harness (`WhisperNpuHal.App --classify`)
...
1. `dump_fixtures.py` snapshots the validators' exact feeds **and** the CPU
   reference probability to `models\deepfake\fixtures\<model>\` (gitignored):
   `model.tsv` / `samples.tsv` / `inputs.tsv` + raw `data\*.bin` tensors.
2. `WhisperNpuHal.App --classify <fixture_dir> [device] [runs]` loads those
   ...
   the output). Writes `results\deepfake-benchmark-cpp.csv` (per-sample p in the
...
.\scripts\get-ort-sdk.ps1                       # DirectML SDK -> third_party\onnxruntime
...
$exe = "build\x64\Release\WhisperNpuHal.App.exe"
& $exe --classify models\deepfake\fixtures\tsc       cpu 20 --results results\deepfake-benchmark-cpp.csv
& $exe --classify models\deepfake\fixtures\tsc       gpu 20 --results results\deepfake-benchmark-cpp.csv
& $exe --classify models\deepfake\fixtures\fakeaudio npu 20 --provider VitisAIExecutionProvider --results results\deepfake-benchmark-cpp.csv
```
...
The VitisAI/QNN/OpenVINO EP branches are wired in `src\classifier\ort_classifier.cpp`
...
.venv\Scripts\python.exe scripts\experiments\benchmark_deepfake_onnx.py
```

Models come from `scripts\get-deepfake-models.ps1` (gitignored under
`models\deepfake\`). The FakeAudio sample clips are gitignored too; if they're
...
| AMD NPU (Ryzen AI / VitisAI) | x64 | **Ryzen AI SDK conda env** (`scripts\setup-amd.ps1`) | `VitisAIExecutionProvider` | Ships its OWN custom `onnxruntime.dll`; don't pip another ORT into that env. fp32 graph needs AMD quantization/compile to actually land on the NPU (see `quantize_int8_vitis.py`). |
...
```powershell
$env:WHISPER_HAL_ORT_DLL_DIR = "C:\Program Files\RyzenAI\1.8.0-beta\deployment"
python scripts\experiments\benchmark_deepfake_onnx.py --providers cpu vitisai
```
...
Select per host via `WHISPER_HAL_ORT_DLL_DIR` (Python, above),
