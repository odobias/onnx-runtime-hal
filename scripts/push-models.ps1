...
# scripts/model-locations.json. This uploads each local path to its hf target
...
$mapPath = Join-Path $PSScriptRoot "model-locations.json"
...
foreach ($e in $entries) {
    $localPath = Join-Path $models ($e.local -replace '/', '\')
    if (-not (Test-Path $localPath)) {
        Write-Host "  ! local missing, skipping: models\$($e.local -replace '/', '\')" -ForegroundColor DarkYellow
        continue
    }
    Write-Host "  models\$($e.local -replace '/', '\')  ->  $($e.hf)" -ForegroundColor DarkGray
