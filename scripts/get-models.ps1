...
# eval/, deepfake/, ...). The canonical map lives in scripts/model-locations.json;
...
$staging = Join-Path $outDir ".hf-staging"
...
foreach ($e in $entries) {
    ...
    $dst = Join-Path $outDir ($e.local -replace '/', '\')
    ...
    Write-Host "  $($e.hf)  ->  models\$($e.local -replace '/', '\')" -ForegroundColor DarkGray
}
