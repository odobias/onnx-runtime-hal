...
Provenance (2026-07-09, from the training repository):
  - Labeled samples: src/wanna_media_nlp/core/validation_samples/{scam,clean}.txt
    from the wanna-media-nlp training/eval repository. These are the exact
    sanity-check sentences the
  ...
  - Caveat: we don't have the base model's tokenizer_config.json /
    special_tokens_map.json (those live in object storage, not the package feed), so
...
# the FakeAudio audio-samples policy. Pulled verbatim from wanna-media-nlp:
