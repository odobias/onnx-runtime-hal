...
DEFAULT_PATH = ROOT / "benchmark" / "baselines" / "whisper-static-multi-7s.json"


def _host_id() -> str:
    value = f"benchmark-host\0{platform.node()}".encode("utf-8")
    return f"host-{hashlib.sha256(value).hexdigest()[:16]}"
