...
benchmark of record is the C++ app driven by benchmark/run-suite.ps1, whose
...
Cross-platform notes (see tools/research/README.md for the full matrix):
  ...
  - Set NPU_INFERENCE_BENCH_ORT_DLL_DIR to prepend a native DLL directory to the
...
_ort_dll_dir = os.environ.get("NPU_INFERENCE_BENCH_ORT_DLL_DIR")
...
def published_path(path):
    if not path:
        return ""
    if path.startswith("${REPO_ROOT}") or path.startswith("external:"):
        return path
    try:
...
def main():
    ...
    for model_key in args.models:
        ...
        for provider_arg in provider_args:
            ...
            row = {
                "timestamp_utc": ts, "model": model_key,
                "model_path": published_path(spec["module"].MODEL_PATH),
