# Fetch the FakeAudio export-smoke WAVs (5 real + 5 synthetic) from a compatible
# source checkout into artifacts/workloads/classifiers/audio-samples/test_audio/.
#
...
#
# Usage:
#   $env:FAKEAUDIO_SAMPLES_REPO_URL = "https://example.test/fakeaudio-samples.git"
...
param(
    [string]$SourceDir = "",
    [string]$RepoUrl = $env:FAKEAUDIO_SAMPLES_REPO_URL,
    [string]$RepoRef = "main",
    [switch]$Force
)
...
$scratchRepo = Join-Path $root "artifacts\scratch\fakeaudio-samples-repo"
...
if (-not $RepoUrl) {
    throw "Pass -RepoUrl, set FAKEAUDIO_SAMPLES_REPO_URL, or provide -SourceDir."
}
