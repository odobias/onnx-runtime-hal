# One-shot / idempotent copy: models/deepfake -> workloads/classifiers
...
$dst = Join-Path $root "workloads\classifiers"
...
if ((Test-Path -LiteralPath $sentinel) -and -not $Force) {
    Write-Host "workloads/classifiers already populated ($sentinel). Pass -Force to overwrite." -ForegroundColor DarkCyan
    exit 0
}
...
Write-Host "Classifier runtime root is now workloads/classifiers (models/deepfake is legacy cache)." -ForegroundColor Green
