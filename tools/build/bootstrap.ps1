...
& (Join-Path $fetchDir "get-audio.ps1")

Write-Host "== Step 5/5: build ==" -ForegroundColor Cyan
& (Join-Path $buildDir "build.ps1") @buildArgs

Write-Host ""
Write-Host "Bootstrap complete ($Platform). Run it with:" -ForegroundColor Green
switch ($Platform) {
    'intel'    { Write-Host "  .\benchmark\run-whisper.ps1 -Backend intel -Device npu" -ForegroundColor White }
    'amd'      { Write-Host "  .\benchmark\run-whisper.ps1 -Backend amd -Device npu" -ForegroundColor White }
    'qualcomm' { Write-Host "  .\benchmark\run-whisper.ps1 -Backend qualcomm -Device npu" -ForegroundColor White }
}
